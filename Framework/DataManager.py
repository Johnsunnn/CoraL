...
class DataManager():
    ...
    def get_dataloder(self, name):
        ...
        if name == 'train_set':
            ...
            if self.config.dataset == 'Peptide Sequence' or 'RNA Sequence':
    ...
    def __load_raw_data(self):
        ...
        if self.mode == 'train-test':
            ...
            elif self.config.dataset == 'RNA Sequence':
                self.__read_meta_dataset_from_dir(self.config.path_meta_dataset)
    ...
    def __process_data(self):
        if self.mode == 'train-test':
            ...
            elif self.config.dataset == 'Peptide Sequence' or 'RNA Sequence':
    ...
    def __load_token2index(self):
        if self.config.dataset == 'RNA Sequence':
            self.token2index = pickle.load(open(self.config.path_sORFtoken2index, 'rb'))
    ...
    def __process_binary_classification_dataset(self):
        ...
        if 'inference dataset' in self.config.dataset:
            ...
            self.test_dataloader = self.__construct_dataloader(test_dataset, self.config.test_batch_size)
            return None

        '''construct_dataloader'''
        self.train_dataloader = self.__construct_dataloader(self.train_dataset, self.config.batch_size)
        self.test_dataloader = self.__construct_dataloader(self.test_dataset, self.config.test_batch_size)
