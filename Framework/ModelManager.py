...
class ModelManager():
    ...
    def init_model(self):
        if self.mode == 'train-test' or self.mode == 'cross validation':
            ...
            elif self.config.model == 'TextRNN':
                self.model = TextRNN.TextRNN(self.config)
            else:
    ...
    def init_optimizer(self):
        if self.config.optimizer == 'AdamW':
            ...
            self.optimizer = torch.optim.SGD(params=self.model.parameters(), lr=self.config.lr,
    ...
    def def_loss_func(self):
        if self.mode == 'train-test' or self.mode == 'cross validation':
            ...
            elif self.config.loss_func == 'contrast loss':
                pass
    ...
    def __SL_train(self, train_dataloader, test_dataloader):
        ...
        criterion_CL = contrast_loss.SupConLoss()
        criterion_CL = criterion_CL.cuda()
        ...
        for epoch in range(self.config.epoch):
            for batch in train_dataloader:
                ...
                if self.config.model == 'Transformer Encoder':
                    ...
                    if self.config.loss_func == 'contrast loss':
                        data_1 = data.clone()
                        data_2 = data.clone()
                        datas = torch.cat([data_1, data_2], dim=0)
                        logits, features = self.model(datas)
                        bsz = label.shape[0]
                    else:
                ...
                if self.config.loss_func == 'contrast loss':
                    ...
                    label_double = torch.cat([label, label_copy], dim=0)
                    a = criterion_CL(features, label)
                    b = criterion_CE(logits.view(-1, self.config.num_class), label_double.view(-1)).mean()
                    train_loss = a + b
                else:
                    train_loss = self.__get_loss(logits, label)
                ...
                if self.config.loss_func == 'contrast loss':
                    if step % self.config.interval_log == 0:
                        print('Epoch[{}] Batch[{}] - Totalloss: {:.6f} CLloss: {:.6f} CEloss: {:.6f}| '
                              .format(epoch, step, train_loss, a, b))
            ...
            if epoch % self.config.interval_test == 0:
                ...
                if self.config.metric == 'ACC':
                    ...
                    if self.config.save_best and test_acc > best_acc:
                        self.IOManager.save_model_dict(self.model.state_dict(),
                                                       self.config.model_save_name + ', epoch[{}]'.format(epoch),
                                                       'ACC', test_acc)
                        best_acc = test_acc
                    ...
                    #                                    self.config.model_save_name + ', epoch[{}]'.format(epoch),
                    #                                    'MCC', test_mcc)
                    if self.config.save_best and test_mcc > best_mcc:
                        self.IOManager.save_model_dict(self.model.state_dict(),
                                                       self.config.model_save_name + ', epoch[{}]'.format(epoch),
                                                       'MCC', test_mcc)
                        best_mcc = test_mcc
        ...
        with torch.no_grad():
            for batch in dataloader:
                ...
                if self.config.model == 'Transformer Encoder':
                    ...
                    print('Mo Such Model')
                # TODO:画降维的图的代码
                # util_plot.draw_umap_plot(features, label, str(index_epoch), self.config.data_name)
                if self.config.loss_func == 'contrast loss':
                    avg_test_loss += criterion(logits, label)
                else:
